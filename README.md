...
- [Advent of Code 2023](#advent-of-code-2023)
  ...
  - [Usage](#usage)
    ...
    - [Run all solutions](#run-all-solutions)
      - [Update readme benchmarks](#update-readme-benchmarks)
    - [Run all tests](#run-all-tests)
    - [Format code](#format-code)
    - [Lint code](#lint-code)
    - [Read puzzle description in terminal](#read-puzzle-description-in-terminal)
  - [Optional template features](#optional-template-features)
    - [Configure aoc-cli integration](#configure-aoc-cli-integration)
    - [Automatically track ⭐️ progress in the readme](#automatically-track-️-progress-in-the-readme)
      - [1. Create a private leaderboard](#1-create-a-private-leaderboard)
      - [2. Set repository secrets](#2-set-repository-secrets)
    - [Check code formatting](#check-code-formatting)
    - [Use VS Code to debug your code](#use-vs-code-to-debug-your-code)
...
![christmas ferris](./.assets/christmas_ferris.png =164x)
...
1. Click [Use this template](https://github.com/fspoettel/advent-of-code-rust/generate) and create your repository.
1. Clone your repository to your computer.
1. If you are solving a previous year's advent of code, change the `AOC_YEAR` variable in `.cargo/config.toml` to
   reflect the year you are solving.

### Setup rust

1. Install the [Rust toolchain](https://www.rust-lang.org/tools/install).
1. (recommended) Install the [rust-analyzer](https://rust-analyzer.github.io/manual.html) extension for your code
   editor.
1. (optional) Install a native debugger. If you are using VS Code,
   [CodeLLDB](https://marketplace.visualstudio.com/items?itemName=vadimcn.vscode-lldb) is a good option.
...
> helper like `let result = part_two(&advent_of_code::template::read_file_part("examples", DAY, 2));` to read it in
> `test_part_two`.
>
> [!TIP]
...
The template can output a table with solution times to your readme. In order to generate a benchmarking table, run
`cargo all --release --time`. If everything goes well, the command will output "_Successfully updated README with
benchmarks._" after the execution finishes and the readme will be updated.

Please note that these are not "scientific" benchmarks, understand them as a fun approximation. 😉 Timings, especially
...
1. Create an `.adventofcode.session` file in your home directory and paste your session cookie. To retrieve the session
...
Once installed, you can use the [download command](#download-inputs-for-a-day), the read command, and
...
leaderboard yet, create one by clicking _Create It_. Your leaderboard should be accessible under
...
- `AOC_SESSION`: an active session[^2] for the advent of code website. To get this, press F12 anywhere on the Advent of
  Code website to open your browser developer tools. Look in your Cookies under the Application or Storage tab, and
  copy out the `session` cookie.
...
✨ You can now run this action manually via the _Run workflow_ button on the workflow page. If you want the workflow
to run automatically, uncomment the `schedule` section in the `readme-stars.yml` workflow file or add a `push` trigger.
...
1. Install [rust-analyzer](https://marketplace.visualstudio.com/items?itemName=rust-lang.rust-analyzer) and
   [CodeLLDB](https://marketplace.visualstudio.com/items?itemName=vadimcn.vscode-lldb).
1. Set breakpoints in your code. [^3]
1. Click _Debug_ next to the unit test or the _main_ function. [^4]
1. The debugger will halt your program at the specific line and allow you to inspect the local stack. [^5]
...
Do you have aoc-specific crate recommendations?
[Share them!](https://github.com/fspoettel/advent-of-code-rust/edit/main/README.md)
...
- **Integer overflows:** This template uses 32-bit integers by default because it is generally faster - for example when
  ...
  might exceed 32-bit integer space. While this is checked and panics in `debug` mode, integers
  [wrap](https://doc.rust-lang.org/book/ch03-02-data-types.html#integer-overflow) in `release` mode, leading to wrong
  output when running your solution.

## Footnotes

[^1]:
    The session cookie might expire after a while (~1 month) which causes the downloads to fail. To fix this issue,
    refresh the `.adventofcode.session` file.

[^2]:
    The session cookie might expire after a while (~1 month) which causes the automated workflow to fail. To fix this
    issue, refresh the AOC_SESSION secret.

[^3]: ![Set a breakpoint](https://user-images.githubusercontent.com/1682504/198838369-453dc22c-c645-4803-afe0-fc50d5a3f00c.png)
[^4]: ![Run debugger](https://user-images.githubusercontent.com/1682504/198838372-c89369f6-0d05-462e-a4c7-8cd97b0912e6.png)
[^5]: ![Inspect debugger state](https://user-images.githubusercontent.com/1682504/198838373-36df6996-23bf-4757-9335-0bc4c1db0276.png)
